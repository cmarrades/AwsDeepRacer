...
def reward_function(params):
    ...
    if not all_wheels_on_track or distanceFromCenterReward == 0 :
        # Penalize if the car goes off track
        reward = 1e-3
        return reward
    else:
        # weight center distance to 0.6 and progress 0.4
        reward = ((closestWaypontsReward * 0.2) + (distanceFromCenterReward * 0.5) + (progressReward * 0.30) + (allWheelsOnTrackReward * 0.5)) * speedPenalization
